...
from bot.objects.embed_builder import daily_guild_report_embed
            ...
            for guild in self.bot.guilds:
                ...
                if discord_guild_db is None:
                    ...
                    channel = self.bot.get_channel(
                        discord_guild_db.announcement_channel_id
                    )
                    ...
                    run_list = await db.get_daily_non_guild_runs(
                        discord_guild_id=discord_guild_db.id,
                        number_of_runs=(8 - previous_run_count),
                    )

                    all_runs = await db.get_all_daily_runs(
                        discord_guild_id=discord_guild_db.id
                    )

                    graph = await visualizer.daily_guild_runs_plot(
                        all_runs, discord_guild_id=discord_guild_db.id
                    )
        ...
        while not self.is_closed():
            for guild in self.bot.guilds:
                ...
                if discord_guild is None:
                    ...
                    try:
                        guild_crawl = await asyncio.wait_for(
                            raiderIO.crawl_discord_guild_members(discord_guild.id),
                            timeout=1200,
                        )
                        await channel.send(guild_crawl)

                        character_crawl = await asyncio.wait_for(
                            raiderIO.crawl_characters(discord_guild.id), timeout=1200
                        )
                        await channel.send(character_crawl)

                    except TimeoutError:
                        print(
                            f"Timeout occurred when trying to crawl data for guild id: {discord_guild.id}"
                        )
                        continue
                    except Exception:
                        print(
                            f"An error occurred when trying to crawl data for guild id: {discord_guild.id}"
                        )
                        continue
