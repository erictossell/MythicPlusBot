...
from bot import visualizer
from bot.objects.embed_builder import (
    daily_guild_report_embed,
    weekly_guild_report_embed,
)
from bot.util import hex_to_rgb
...
class Guild(commands.Cog):
    ...
    async def daily_report(self, ctx):
        ...
        try:
            async with ctx.typing():
                ...
                guild_run_list = await db.get_daily_guild_runs(
                    discord_guild_id=ctx.guild.id
                )
                run_list = await db.get_daily_non_guild_runs(
                    discord_guild_id=ctx.guild.id,
                    number_of_runs=(8 - len(guild_run_list)),
                )

                all_runs = await db.get_all_daily_runs(discord_guild_id=ctx.guild.id)

                graph = await visualizer.daily_guild_runs_plot(
                    all_runs, discord_guild_id=ctx.guild.id
                )

                embed = daily_guild_report_embed(
    ...
    async def weekly_report(self, ctx):
        ...
        try:
            async with ctx.typing():
                ...
                guild_run_list = await db.get_top10_guild_runs_this_week(
                    discord_guild_id=ctx.guild.id
                )

                run_list = await db.get_weekly_non_guild_runs(
                    discord_guild_id=ctx.guild.id,
                    number_of_runs=(8 - len(guild_run_list)),
                )
                ...
                graph = await visualizer.weekly_guild_runs_plot(
                    all_runs, guild_run_list, discord_guild_id=ctx.guild.id
                )

                embed = weekly_guild_report_embed(
                    discord_guild_db=discord_guild_db,
                    guild_run_list=guild_run_list,
                    non_guild_run_list=run_list,
                    bot_user=bot_user,
    ...
    async def top_runs(self, ctx):
        ...
        try:
            ...
            dungeon_list = await db.get_top5_guild_runs_all_time(
                discord_guild_id=ctx.guild.id
            )
            ...
            embed = discord.Embed(
                ...
                color=discord.Color.from_rgb(*hex_to_rgb("#c300ff")),
            )
            ...
            for run, characters in dungeon_list:
                ...
                embed.add_field(
                    ...
                    + "  |  +"
                    + str(run.num_keystone_upgrades),
                    value=run_characters + f"\n[Link to run]({run.url})",
                    inline=False,
                )
                counter += 1
    ...
    async def leaderboard_embed(self, ctx, leaderboard_type):
        ...
        if leaderboard_type == "mythic_plus":
            ...
            field_func = lambda leader: (
                f"{leader.score} - {leader.name} | {leader.spec_name} - {leader.class_name}",
                f"[M+ Class Rank on {leader.realm.capitalize()}: {leader.rank}]({leader.url})",
            )
            ...
            field_func = lambda leader: (
                f"{leader.achievement_points} - {leader.name} | {leader.spec_name} - {leader.class_name}",
                f"Last updated: [{leader.last_crawled_at}]({leader.url})",
            )
            ...
            field_func = lambda leader: (
                f"{leader.item_level} - {leader.name} | {leader.spec_name} - {leader.class_name}",
                f"[M+ Class Rank on {leader.realm.capitalize()}: {leader.rank}]({leader.url})",
            )
        ...
        for index, leader in enumerate(characters_list, start=1):
            ...
            embed.add_field(
                name=f"{index}. {field_name}", value=field_value, inline=False
            )
