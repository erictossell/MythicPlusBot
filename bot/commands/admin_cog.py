...
import bot.db as db
from bot.objects.guild_registration import RegisterGuildView
...
class Admin(commands.Cog):
    ...
    async def register(self, ctx):
        ...
        try:
            if ctx.author.guild_permissions.administrator:
                ...
                await channel.send(
                    view=RegisterGuildView(discord_guild_id=ctx.guild.id)
                )
            else:
                await logger.build_manual_log(
                    ...
                    "admin_cog",
                    "register",
                    f"User {ctx.author.name} attempted to run !register command but is not an admin of {ctx.guild.name}.",
                )
                await ctx.respond(
                    "You are not an administrator, please contact your admin to run this command.."
                )

        except Exception as e:
            print(e)
            await ctx.respond(
                "Something went wrong :( Talk to Eriim about this error. "
            )
            ...
            await logger.build_manual_log(
                logger, "admin_cog", "register", f"Error in !register command: {e}"
            )
    ...
    async def set_announcement_channel(self, ctx):
        ...
        if ctx.author.guild_permissions.administrator:
            if discord_guild is None:
                await ctx.respond(
                    "This command can only be used by admins within a Discord Server."
                )
                return
            ...
            await logger.build_manual_log(
                ...
                f"User {ctx.author.name} set announcement channel to {ctx.channel.name}.",
            )
        else:
            await ctx.respond(
                "You are not an administrator, please contact your admin to run this command.."
    ...
    async def disable_announcements(self, ctx):
        ...
        if ctx.author.guild_permissions.administrator:
            ...
            await logger.build_manual_log(
                ...
                f"User {ctx.author.name} disabled announcements.",
            )
        else:
            await ctx.respond(
                "You are not an administrator, please contact your admin to run this command.."
        ...
        if ctx.author.guild_permissions.administrator:
            if discord_guild is None:
                ...
                await logger.build_manual_log(
                    ...
                    "admin_cog",
                    "set_players_per_run",
                    f"User {ctx.author.name} set players per run to {players_per_run} for {ctx.guild.name}.",
                )
        else:
            await ctx.respond(
                "You are not an administrator, please contact your admin to run this command.."
    ...
    async def on_ready(self):
        ...
        await logger.build_manual_log(
            logger, "admin_cog", "on_ready", f"Connected to Discord as {self.bot.user}."
        )
        await db.create_schema()
        for guild in self.bot.guilds:
            await db.add_discord_guild(
                db.DiscordGuildDB(id=guild.id, discord_guild_name=guild.name)
            )
            await logger.build_manual_log(
                logger,
                "admin_cog",
                "on_ready",
                f"Validated guild: {guild.name} to database.",
            )
            print(
    ...
    async def on_guild_join(self, guild):
        print(
            f"{self.bot.user} has joined the following guild:\n"
            f"{guild.name}(id: {guild.id})"
        )
        await db.add_discord_guild(
            db.DiscordGuildDB(id=guild.id, discord_guild_name=guild.name)
        ...
        await guild.system_channel.send(
            "Hello! I am Mythic+ Bot. I provide advanced Discord reporting for Mythic+ data.\nPlease go to https://www.mythicplusbot.dev/ if you would like guidance setting some of my features up 😊."
        )
        await logger.build_manual_log(
            logger,
            "admin_cog",
            "on_guild_join",
            f"Joined guild: {guild.name} to database.",
        )
