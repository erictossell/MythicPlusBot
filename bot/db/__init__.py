...
from bot.db.models.discord_guild_run_db import DiscordGuildRunDB
from bot.db.models.discord_user_character_db import DiscordUserCharacterDB
from bot.db.models.discord_guild_character_db import DiscordGuildCharacterDB

from bot.db.models.character_db import CharacterDB
from bot.db.models.character_history_db import CharacterHistoryDB

from bot.db.models.dungeon_run_db import DungeonRunDB
from bot.db.models.character_run_db import CharacterRunDB

from bot.db.models.announcement_db import AnnouncementDB
...
async def get_discord_game_guild_by_guild_ids(
    discord_guild_id: int, game_guild_id: int
) -> Optional[DiscordGameGuildDB]:
    try:
        async with async_session_scope() as session:
            query = select(DiscordGameGuildDB).filter(
                DiscordGameGuildDB.discord_guild_id == discord_guild_id,
                DiscordGameGuildDB.game_guild_id == game_guild_id,
            )
...
async def get_discord_guild_character_by_name(
    discord_guild_id: int, name: str
) -> Optional[DiscordGuildCharacterDB]:
    try:
        async with async_session_scope() as session:
            query = (
...
async def get_character_by_name_realm_and_discord_guild(
    name: str, realm: str, discord_guild_id: int
...
async def get_game_guild_by_name_realm(name: str, realm: str) -> Optional[GameGuildDB]:
    try:
        async with async_session_scope() as session:
            query = select(GameGuildDB).filter(
                GameGuildDB.name == name, GameGuildDB.realm == realm
            )
            result = await session.execute(query)
...
async def get_discord_guild_by_id(discord_guild_id: int) -> Optional[DiscordGuildDB]:
    ...
    try:
        async with async_session_scope() as session:
            existing_guild_query = select(DiscordGuildDB).filter(
                DiscordGuildDB.id == int(discord_guild_id)
            )
    ...
    try:
        async with async_session_scope() as session:
            run_query = (
                ...
                .filter(
                    DungeonRunDB.dungeon_id == run_id, DungeonRunDB.season == season
                )
            )
...
async def get_character_run_by_character_run(
    character: CharacterDB, run: DungeonRunDB
) -> CharacterRunDB:
    ...
    try:
        async with async_session_scope() as session:
            ec_query = select(CharacterDB).filter(
                CharacterDB.name == character.name, CharacterDB.realm == character.realm
            )
            ...
            if existing_character is None:
                ...
                ecr_query = select(CharacterRunDB).filter(
                    ...
                    CharacterRunDB.run_id == existing_run.id,
                )
...
async def get_discord_user_character_by_guild_user(
    ...
    try:
        async with async_session_scope() as session:
            discord_user_character_query = select(DiscordUserCharacterDB).filter(
            ...
            if discord_user_character is not None:
                character_query = select(CharacterDB).filter(
                    CharacterDB.id == discord_user_character.character_id
                )
    ...
    try:
        async with async_session_scope() as session:
            ...
            query = (
                ...
                .options(
                    joinedload(AnnouncementDB.dungeon_run)
                )  # eager load dungeon_run
                .join(DungeonRunDB.announcements)
                .filter(
                    AnnouncementDB.discord_guild_id == discord_guild_id,
                    DungeonRunDB.completed_at >= one_week_ago,
                    AnnouncementDB.has_been_sent == False,
                )
...
async def add_discord_guild_character(
    discord_guild: DiscordGuildDB, character: CharacterDB
):
    try:
        async with async_session_scope() as session:
            guild_query = select(DiscordGuildDB).filter(
                DiscordGuildDB.id == discord_guild.id
            )
            ...
            if existing_guild:
                ...
                if existing_character:
                    guild_character_query = select(DiscordGuildCharacterDB.id).filter(
                        DiscordGuildCharacterDB.discord_guild_id == existing_guild.id,
                        DiscordGuildCharacterDB.character_id == existing_character.id,
                    )
                    guild_character_result = await session.execute(
...
async def add_discord_guild_run(
    discord_guild: DiscordGuildDB, dungeon_run: DungeonRunDB
) -> DiscordGuildRunDB:
    try:
        async with async_session_scope() as session:
            guild_query = select(DiscordGuildDB).filter(
            ...
            if existing_guild:
                run_query = select(DungeonRunDB).filter(
                    DungeonRunDB.id == dungeon_run.id
                )
                run_result = await session.execute(run_query)
                existing_run = run_result.scalar()

                if existing_run:
                    guild_run_query = select(DiscordGuildRunDB).filter(
                        DiscordGuildRunDB.discord_guild_id == discord_guild.id,
                        DiscordGuildRunDB.dungeon_run_id == dungeon_run.id,
                    )
                    ...
                    if existing_guild_run:
                        ...
                        new_guild_run = DiscordGuildRunDB(
...
async def add_discord_game_guild(
    ...
    try:
        async with async_session_scope() as session:
            query = select(DiscordGameGuildDB).filter(
                DiscordGameGuildDB.discord_guild_id
                == discord_game_guild.discord_guild_id,
                DiscordGameGuildDB.game_guild_id == discord_game_guild.game_guild_id,
            )
            result = await session.execute(query)
            existing_game_guild = result.scalar()
...
async def add_character(character: CharacterDB) -> CharacterDB:
    ...
    try:
        async with async_session_scope() as session:
            query = select(CharacterDB).filter(
                CharacterDB.name == character.name, CharacterDB.realm == character.realm
            )
            result = await session.execute(query)
            existing_character = result.scalar()
...
async def add_dungeon_run(dungeon_run: DungeonRunDB) -> DungeonRunDB:
    ...
    try:
        ...
        async with async_session_scope() as session:
            query = select(DungeonRunDB).filter(
                DungeonRunDB.dungeon_id == int(dungeon_run.dungeon_id),
                DungeonRunDB.season == dungeon_run.season,
            )
            result = await session.execute(query)
...
async def add_character_run(character_run: CharacterRunDB) -> Optional[CharacterRunDB]:
    try:
        async with async_session_scope() as session:
            ec_query = select(CharacterDB).filter(
                CharacterDB.name == character_run.character.name,
                CharacterDB.realm == character_run.character.realm,
            )
            ec_result = await session.execute(ec_query)
            existing_character = ec_result.scalar()

            er_query = select(DungeonRunDB).filter(
                DungeonRunDB.id == character_run.dungeon_run.id
            )
            er_result = await session.execute(er_query)
            existing_run = er_result.scalar()
...
async def add_discord_user_character(
    ...
    try:
        async with async_session_scope() as session:
            er_query = select(DiscordUserCharacterDB).filter(
                DiscordUserCharacterDB.discord_user_id
                == discord_user_character.discord_user_id
            )
            er_result = await session.execute(er_query)
...
async def add_announcement(announcement: AnnouncementDB) -> bool:
    ...
    try:
        async with async_session_scope() as session:
            er_query = select(DungeonRunDB).filter(
                DungeonRunDB.id == announcement.dungeon_run_id
            )
            er_result = await session.execute(er_query)
            ...
            new_announcement = AnnouncementDB(
                discord_guild_id=announcement.discord_guild_id,
    ...
    try:
        async with async_session_scope() as session:
            ...
            if not existing_guild:
                ...
                existing_guild.announcement_channel_id = (
                    discord_guild.announcement_channel_id
                )
...
async def update_discord_guild_character(
    ...
    try:
        async with async_session_scope() as session:
            query = select(DiscordGuildCharacterDB).filter(
                DiscordGuildCharacterDB.discord_guild_id
                == discord_guild_character.discord_guild_id,
                DiscordGuildCharacterDB.character_id
                == discord_guild_character.character_id,
            )
            result = await session.execute(query)
            existing_discord_guild_character = result.scalar()
            if not existing_discord_guild_character:
                ...
                existing_discord_guild_character.guild_character_score = (
                    discord_guild_character.guild_character_score
                )
                existing_discord_guild_character.is_reporting = (
                    discord_guild_character.is_reporting
                )
...
async def update_character(character: Character) -> CharacterDB:
    ...
    try:
        async with async_session_scope() as session:
            existing_character_query = select(CharacterDB).filter(
                CharacterDB.name == character.name, CharacterDB.realm == character.realm
            )
            result = await session.execute(existing_character_query)
...
async def update_character_reporting(character: Character) -> bool:
    ...
    print(
        f"Tal_DB : updating character reporting status: {character.name} on realm: {character.realm}"
    )
    try:
        async with async_session_scope() as session:
            ec_query = (
                ...
                .join(
                    DiscordGuildCharacterDB,
                    CharacterDB.id == DiscordGuildCharacterDB.character_id,
                )
                .filter(
                    CharacterDB.name == character.name,
                    CharacterDB.realm == character.realm,
                )
            )
            ec_result = await session.execute(ec_query)
            existing_character = ec_result.scalar()
            if existing_character is not None:
                ...
                ) in existing_character.discord_guild_characters:
...
async def update_discord_user_character(
    discord_user_id: int, character: CharacterDB
) -> DiscordUserCharacterDB:
    ...
    try:
        async with async_session_scope() as session:
            ...
            nc_query = select(CharacterDB).filter(
                CharacterDB.name == character.name, CharacterDB.realm == character.realm
            )
    ...
    try:
        async with async_session_scope() as session:
            ...
            if existing_announcement is not None:
                ...
                print(
                    "Tal_DB : updated announcement_has_been_sent for announcement: "
                    + str(announcement_id)
                )
                return True
            else:
...
async def remove_character(name, realm) -> bool:
    ...
    try:
        async with async_session_scope() as session:
            query = select(CharacterDB).filter(
                CharacterDB.name == name, CharacterDB.realm == realm
            )
            result = await session.execute(query)
            existing_character = result.scalar()
            if existing_character is not None:
...
async def remove_discord_user_character(
    ...
    try:
        async with async_session_scope() as session:
            query = select(DiscordUserCharacterDB).filter(
                DiscordUserCharacterDB.discord_guild_id
                == discord_user_character.discord_guild_id,
                DiscordUserCharacterDB.discord_user_id
                == discord_user_character.discord_user_id,
            )
            result = await session.execute(query)
    ...
    try:
        async with async_session_scope() as session:
            ...
            subquery = (
                ...
                .join(
                    DiscordGuildCharacterDB,
                    DiscordGuildCharacterDB.character_id == CharacterDB.id,
                )
                .filter(
                    DungeonRunDB.completed_at > datetime.utcnow() - timedelta(days=1),
                    DiscordGuildCharacterDB.discord_guild_id == discord_guild_id,
                )
            ...
            )  # Add this line to create the alias
    ...
    try:
        async with async_session_scope() as session:
            ...
            subquery = (
                ...
                .filter(
                    DungeonRunDB.completed_at > datetime.utcnow() - timedelta(weeks=1),
                    DiscordGuildCharacterDB.discord_guild_id == discord_guild_id,
                )
                .order_by(desc(DungeonRunDB.score))
                .distinct()
...
async def get_daily_non_guild_runs(
    discord_guild_id: int, number_of_runs: int
) -> Optional[DefaultDict[List[DungeonRunDB], List[CharacterDB]]]:
    try:
        async with async_session_scope() as session:
            subquery = (
...
async def get_weekly_non_guild_runs(
    discord_guild_id: int, number_of_runs: int
) -> Optional[DefaultDict[List[DungeonRunDB], List[CharacterDB]]]:
    try:
        async with async_session_scope() as session:
...
async def get_characters_by_names_realms_and_discord_guild(
    names: List[str], realms: List[str], discord_guild_id: int
) -> Optional[List[CharacterDB]]:
    try:
        async with async_session_scope() as session:
            query = (
    ...
    try:
        async with async_session_scope() as session:
            characters_query = (
                ...
                .join(
                    CharacterDB.discord_guild
                )  # Join the tables using the relationship
            )
...
async def get_all_characters_active_in_last_30_days() -> List[CharacterDB]:
    ...
    try:
        async with async_session_scope() as session:
            characters_query = select(CharacterDB).filter(
                CharacterDB.last_crawled_at > datetime.now() - timedelta(days=30),
                CharacterDB.is_reporting == True,
            )
            result = await session.execute(characters_query)
            characters = result.scalars().unique().all()
...
async def get_all_characters_not_recently_crawled() -> List[CharacterDB]:
    ...
    try:
        async with async_session_scope() as session:
            characters_query = select(CharacterDB).filter(
                CharacterDB.last_crawled_at < datetime.now() - timedelta(days=1),
                CharacterDB.is_reporting == True,
            )
            result = await session.execute(characters_query)
            characters = result.scalars().unique().all()
...
async def get_all_run_ids_by_season(season: str) -> List[int]:
    ...
    try:
        async with async_session_scope() as session:
            runs_query = select(DungeonRunDB.dungeon_id).filter(
    ...
    try:
        async with async_session_scope() as session:
            ...
            if existing_run is None:
                ...
                existing_character_runs_query = select(CharacterRunDB).filter(
                    CharacterRunDB.dungeon_run_id == existing_run.id
                )
                excresult = await session.execute(existing_character_runs_query)
                existing_character_runs = excresult.scalars().all()

                existing_characters = select(CharacterDB).filter(
                    CharacterDB.id.in_(
                        [char.character_id for char in existing_character_runs]
                    )
                )
        ...
        async with async_session_scope() as session:
            ...
            if existing_character is None:
                ...
                existing_character_runs_query = (
                    ...
                    .join(
                        DungeonRunDB, CharacterRunDB.dungeon_run_id == DungeonRunDB.id
                    )
                ...
                if not existing_character_runs:
                    ...
                    existing_runs_query = (
                        ...
                        .join(
                            CharacterRunDB,
                            DungeonRunDB.id == CharacterRunDB.dungeon_run_id,
                        )
                        .join(
                            CharacterDB, CharacterRunDB.character_id == CharacterDB.id
                        )
                        .filter(
                            DungeonRunDB.id.in_(
                                [run.dungeon_run_id for run in existing_character_runs]
                            )
                        )
                        .order_by(DungeonRunDB.score.desc())
                    )
...
def has_any_character_field_changed(
    existing_character: CharacterDB, character: Character
) -> bool:
    return (
        existing_character.guild_name != character.guild_name
        or existing_character.realm != character.realm.lower()
        or existing_character.faction != character.faction
        or existing_character.region != character.region
        or existing_character.role != character.role
        or existing_character.spec_name != character.spec_name
        or existing_character.class_name != character.class_name
        or existing_character.achievement_points != character.achievement_points
        or existing_character.item_level != character.item_level
        or existing_character.score != character.score
        or existing_character.rank != character.rank
        or existing_character.thumbnail_url != character.thumbnail_url
        or existing_character.url != character.url
        or existing_character.last_crawled_at != character.last_crawled_at
        or existing_character.is_reporting != character.is_reporting
    )


async def check_run_for_guild_run(
    discord_guild_id: int, dungeon_run_id: int
) -> Optional[DiscordGuildRunDB]:
    try:
        async with async_session_scope() as session:
            dungeon_query = select(DungeonRunDB.id).filter(
                DungeonRunDB.dungeon_id == dungeon_run_id
            )
            dungeon_run_result = await session.execute(dungeon_query)
            dungeon_run_id = dungeon_run_result.scalar()

            subquery = select(CharacterRunDB.character_id).filter(
                CharacterRunDB.dungeon_run_id == dungeon_run_id
            )

            query = select(func.count(DiscordGuildCharacterDB.character_id)).filter(
            ...
            players_per_run = await session.execute(
                select(DiscordGuildDB.players_per_run).filter(
                    DiscordGuildDB.id == discord_guild_id
                )
            )
            ...
            if guild_character_count_value >= players_per_run_value:
                guild_run_exists = await session.execute(
                    select(DiscordGuildRunDB.id).filter(
                        DiscordGuildRunDB.discord_guild_id == discord_guild_id,
                ...
                if not guild_run_exists:
                    new_guild_run = DiscordGuildRunDB(
                        discord_guild_id=discord_guild_id, dungeon_run_id=dungeon_run_id
                    )
                    session.add(new_guild_run)

                    print(
                        f"New GuildRun created for guild ID {discord_guild_id} and run ID {dungeon_run_id}."
                    )
                    return new_guild_run
                else:
