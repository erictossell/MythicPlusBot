...
def announce_guild_run_embed(
    announcement: db.AnnouncementDB = None,
    ...
    """
    embed = discord.Embed(
        title=announcement.title, description=announcement.content, color=color
    )
    if characters is not None:
        for character in characters:
            embed.add_field(
                name=f"{character.name} - {character.class_name}",
                value=f"Score: {character.score}\nClass Rank: {character.rank}",
                inline=False,
            )
    return embed


def daily_guild_report_embed(
    ...
    embed = discord.Embed(
        ...
        color=discord.Color.from_rgb(*util.hex_to_rgb("#c300ff")),
    )

    embed.set_author(
        name="Mythic+ Bot",
        icon_url=bot_user.avatar,
        url="https://www.mythicplusbot.dev/",
    )
    embed.add_field(
        name="------- Top Guild Runs in the last 24 hours -------",
        value="",
        inline=False,
    )
    counter = 1
    ...
    for run, character_runs in guild_run_list:
        ...
        if over_under > 0:
            embed.add_field(
                name=str(counter)
                + ".  "
                + str(run.mythic_level)
                + (plus)
                + " | "
                + run.name,
                value=guild_run_characters
                + f"\n[{util.time_without_leading_zeros(util.convert_millis(run.clear_time_ms))}/{util.time_without_leading_zeros(util.convert_millis(run.par_time_ms))} - {util.time_without_leading_zeros(util.convert_millis(over_under))} remaining.]({run.url})",
                inline=False,
            )
        else:
            ...
            embed.add_field(
                ...
                + run.name,
                value=guild_run_characters
                + f"\n[{util.time_without_leading_zeros(util.convert_millis(run.clear_time_ms))}/{util.time_without_leading_zeros(util.convert_millis(run.par_time_ms))} - {util.time_without_leading_zeros(util.convert_millis(over_under))} over time.]({run.url})",
                inline=False,
            )
    ...
    embed.add_field(
        name="--------- Top Runs in the last 24 hours ---------", value="", inline=False
    )
...
def weekly_guild_report_embed(
    ...
    embed.add_field(
        name="------- Top Guild Runs in the last 7 days -------", value="", inline=False
    )
    counter = 1
    if len(guild_run_list) == 0:
    ...
    embed.add_field(
        name="--------- Top Runs in the last 7 days ---------", value="", inline=False
    )
    if len(non_guild_run_list) == 0:
        embed.add_field(name="No runs for this week.", value="", inline=False)
...
def character_recent_runs(character: Character, bot_user: discord.User = None):
    """Creates an embed for a character's recent runs."""
    title = character.name.capitalize() + "'s Recent Mythic+ Runs"
    embed = discord.Embed(
        title=title,
        description="",
        color=discord.Color.from_rgb(*util.hex_to_rgb(character.score_color)),
        url=character.url,
    )
    embed.set_author(
        name="Mythic+ Bot",
    ...
    for run in character.recent_runs:
        ...
        if over_under > 0:
            over_under_string = util.time_without_leading_zeros(
                util.convert_millis(over_under)
            )
...
def character_best_runs(character: Character, bot_user: discord.User = None):
    title = character.name.capitalize() + "'s Best Mythic+ Runs"
    embed = discord.Embed(
        title=title,
        description="",
        color=discord.Color.from_rgb(*util.hex_to_rgb(character.score_color)),
