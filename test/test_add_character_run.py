...
from db import CharacterDB, DungeonRunDB, CharacterRunDB, add_character_run, session_scope
...
def test_add_character_run(mock_session):
    ...
    mock_session.query(CharacterDB).filter().first.return_value = character
    mock_session.query(DungeonRunDB).filter().first.return_value = run
    mock_session.query(CharacterRunDB).filter().first.return_value = None

    # Call the add_character_run function
    result = add_character_run(character, run)
    ...
    mock_session.query(CharacterDB).filter.assert_called_once()
    mock_session.query(DungeonRunDB).filter.assert_called_once()
    mock_session.query(CharacterRunDB).filter.assert_called_once()
