...
class getMythicPlusRecentRuns:
    def __init__(self, name, realm):
        self.region = 'us'
        self.realm = realm
        self.name = name        
        self.character = requests.get('https://raider.io/api/v1/characters/profile?region='+self.region+'&realm='+self.realm+'&name='+self.name+'&fields=mythic_plus_recent_runs')
        self.recentRuns = self.character.json()['mythic_plus_recent_runs']
